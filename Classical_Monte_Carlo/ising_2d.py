...
class ising_model:
    """
    construe the 2D ising model
    (nx, ny) = dimension
    J = -1 (ferro) or 1 (anti-ferro)
    ==> model.run_mc(T, mcset, total_mc_step, Do_Autocorrelation_time = True or False)
    ...
    def run_mc(self, T, mcset, total_mc_step, Do_Autocorrelation_time = False):
        beta = 1.0/T
        ...
        for p2 in range(mcset):
            ...
            for p1 in range(total_mc_step*self.nx*self.ny):
                x1, x2 = np.random.randint(self.nx), np.random.randint(self.ny)
                s_change = s[x1, x2]
                # let's set it as square lattice
                neighbr = s[(x1 + 1) % self.nx, x2] + s[x1, (x2 + 1) % self.ny] + s[(x1 - 1) % self.nx, x2] + s[x1, (x2 - 1) % self.ny]
                # new_E - old_E
                cost_energy = -2*self.J*s_change*neighbr
                if rand() < np.exp(-cost_energy*beta):
...
def testing():
    # (nx, ny) = (8, 8) AND anti-ferro
    # check form T = 1.5 to 4
    test_model = ising_model(8, 8, 1)
    ...
    tasks = [(t_list[x], 8, 1000, False) for x in range(m)]
...
def testing_auto_corr_time():
    ...
    # check form T = 1.5 to 4
    n = 20
    test_model = ising_model(10, 10, 1)
    print(test_model.__doc__)
    m = 5
    total_mc = 1000
    ...
    for p1 in range(m):
        data1[p1, :], data2[p1, :]= test_model.run_mc(T = 2.65, mcset = 1, total_mc_step = total_mc, Do_Autocorrelation_time = True)
