...
def testing():
    ...
    tasks = [(t_list[x], cores, 500, 'wolff') for x in range(m)]
...
def testing_auto_corr_time():
    ...
    for p1 in range(m):
        data1[p1, :], data2[p1, :]= test_model.run_mc(T = 2.65, mcset = 1, total_mc_step = total_mc, 'wolff', Do_Autocorrelation_time = True)
    ...
    save = False
    if( save == True ):
        np.savetxt('data_2d_auto_corr_time.csv', data_out, delimiter=',')
